...
impl<'a> InferContext<'a> {
            ...
            fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
                ...
                match self.ctx.get(self.id) {
                    ...
                    Func(args, ret) => write!(
                        ...
                        "({}) -> {}",
                        args.iter()
                            .map(|ty| format!("{}", self.with_id(*ty, true)))
                            .collect::<Vec<_>>()
                            .join(", "),
                        self.with_id(ret, true)
                    ),
                }
            }
        }
    ...
    pub fn unify_inner(
        ...
        match (self.get(a), self.get(b)) {
            ...
            (Func(ai, ao), Func(bi, bo)) if ai.len() == bi.len() => ai
                .into_iter()
                .zip(bi.into_iter())
    ...
    fn reconstruct_inner(
        ...
        let ty = match self.get(id) {
            ...
            Func(args, b) => Type::Func(
                args.into_iter()
                    .map(|item| self.reconstruct_inner(iter + 1, item))
                    .collect::<Result<_, _>>()?,
                self.reconstruct_inner(iter + 1, b)?,
            ),
        };
...
mod tests {
    ...
    fn basic_1() {
        ...
        // Create some types
        let number = ctx.add_scalar(ScalarInfo::Concrete(ScalarType::Float));
        let boolean = ctx.add_scalar(ScalarInfo::Concrete(ScalarType::Bool));

        let a = ctx.insert(TypeInfo::Scalar(number), Span::none());
        let b = ctx.insert(TypeInfo::Unknown, Span::none());
        let c = ctx.insert(TypeInfo::Func(vec![a], b), Span::none());
        let d = ctx.insert(TypeInfo::Scalar(boolean), Span::none());
        ctx.unify(b, d).unwrap();

        assert_eq!(
            ctx.reconstruct(c, Span::none()).unwrap().into_inner(),
            Type::Func(
                vec![SrcNode::new(Type::Scalar(ScalarType::Float), Span::none())],
                SrcNode::new(Type::Scalar(ScalarType::Bool), Span::none()),
            ),
        );
    }
